...
spellings, [proto.record.trap]) and `with-file-index.json` (the s181
additive `files` array and per-diagnostic `file`, [proto.record.diag])
validate; `wrong-version.json` and
