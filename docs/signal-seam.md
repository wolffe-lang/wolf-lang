...
byte to a self-pipe, and a drain thread delivers the event to the
parked task (`[os.signal.model]`).
...
wws wraps these bits with named constants in its own module; the
surface is the builtin ABI and the names are wws's, the way std.process
wraps `os_spawn`. New meanings append with the next free bit; nothing
renumbers.
...
- Linux: full set, the running implementation (gated with the native
  concurrency layer; `reactor`/`net`/`task` are Linux at this campaign
  stage). macOS/BSD delivery widens with the task-layer port.
- Windows (tier-1): POSIX signals do not exist. The meanings map to
  ...
  ws04's Windows reload/upgrade uses its control channel (the ungated
  half) instead of a signal. Console delivery lands with the Windows
  native backend, and the mapping is spec'd now so the seam is a
  meaning seam.
