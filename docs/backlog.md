...
server trace, or a Windows box). The s57 sprint owns it and rebuilds
this layer.
...
The cost is measurable. In `alias_daxpy` the two buffers arrive as a `mut`
and a `read` parameter. Corrected at s78 (wolf-lang#82): the c04 mode
theorems do not prove those two buffers disjoint. `wolf_mem`'s place
...
pointers takes a new theorem; it is not a mechanical transfer. Until c04
proves container-payload disjointness, emitting the pair would be an unsound
`noalias`, the class of bug the fact rig exists to prevent.
...
- The sentinel's resolution is the other half of the +0.0%. At the
  ...
  quantized to 17-50%. Raising the sweep counts ~100x made the readings
  above possible; the manifest sizes date from before s75 made the
...
region without receiving a token, and `@stencil` does that. No dynamic
witness was produced (the shapes that would show it inline first), so this
is a hazard to audit, not a filed miscompile; the sprint refused to hand
the same claim to LLVM as call-site `!noalias`. Audited at s80, below.

## the foreign-root audit and what it turned up (2026-08-13, s80, #83)

The s80 audit went looking for the #83 miscompile and found a different
one, live and reachable from source, in a fact this file never listed
because nobody had doubted it: one `!alias.scope` per region id.

`region.foreign` roots are per function. The inliner freshens every
callee-internal region id, which is correct for `region.new` and
`stack.alloc`, where the callee's arena really is its own. A foreign root
is neither, so inlining any container-touching callee left the caller
holding two roots over the same element buffers, and one-scope-per-region
declared them
...
The fix: for runtime-owned storage the aliasing unit is the role, not the
region identity. `region.foreign` carries a role immediate now
...
- the LLVM emitter gives all same-role foreign regions one `!alias.scope`;
  header-vs-buffer separation is untouched, which is the D46 theorem and
  all s75 needed.
...
fix. It stays open as the way to claim the fact; today's rule only stops
the code relying on it.

#83's own hazard is real, and still not reachable from source. The
...
stops today's lowering from emitting it is worth writing down, because it
is an accident and not a theorem:
...
- a `mut List` argument spills its header pointer to a stack slot and
  ...
  token changes. Either way the address saves it, not the token.
...
Cost, per kernel. The mid-end rules cost zero: optimized WIR is
...
they decline are the shapes the lowering does not currently produce, the
same fact as "no source witness". The scope-class change removes 0-2
...
ms, `list_alloc` 6805 vs 7000 ms. All are inside the noise in both
directions.
...
The s83 sprint wrote the inventory s80 owed: what token propagation does
at each of ordinary calls, `main`, exports, `c_call`, task entry shims,
task/proc bodies, runtime shims, and indirect calls (none exist). Two
things fell out of writing it, and together they are why the propagation
is a successor sprint and not this one.
...
First, uniform propagation restores no optimization by itself. If every
...
Second, the propagation has a trapdoor that must land with it.
...
emittable, since local regions' tokens are exhaustive. A call now carries
...
stands, now for a stated reason. The emitter imports the predicate
instead of re-deriving it, so the fact rig and the passes cannot
drift.
...
= a` moving. Neither is a claim about tokens; both are now tests that
fail if they go.

Cost, per kernel: zero. Optimized WIR is byte-identical with
...
the lowering does not currently produce, the same fact as "no source
witness", so the fuzz shape is the witness.

The new fact buys nothing on those kernels either. Emitted call-site
`!noalias` count at `--release`: 0 of 27-31 calls, on every one of the
thirteen. The kernels' region traffic is container traffic, and
container storage is the foreign case the fact declines. The theorem is
right, the channel works
...
lands. That is the measurement of what the ABI change is worth, taken
...
- the last bounds guard (`i <u out.len`): the guard relates `i` to
  ...
  (a limit against another loop-invariant value, not a constant `K`). The
  affine channel cannot chain it for free; it reasons about one base pair,
  and this needs transitivity across two pairs (a difference-bound
  closure).
- the two overflow checks: the loaded elements have no range, so nothing
  bounds `src[i-1] + src[i]`. X3's claw-back would need a `range` fact on
  container element loads (the values were stored masked), or versioning
...
up to the element's alignment (array-of-struct layout), so element bases
and every field's alignment class are uniform across the buffer. Interior
