...
`bc1018e` (lupin 0.1.6). s71 and lupin v0.1.7 run concurrently, so rows
they own take the **pending-named-work** status. A red row blocks the
tag.

Verdict key: **GREEN** (verified true) · **PENDING(owner)** (named
concurrent work) · **RED(owner)** (blocks the tag; must be fixed,
filed, or ruled, and never waived).
...
- wolf-interp **#16** "silent wrong answer: a function returning an
  ENUM through an error row always takes the miss path". Silent
  wrong answer. Owner: lupin v0.1.7 → pending.
- wolf-interp **#21** "literal-sticky container typing defaults i32
  against the locked 64-bit int (the #53 mechanism)". It misses
  mandated X3 traps (see Row 3's `overflow_elem_write.lu` divergence:
  lupin `exit(0)` where the directive and wolfc say `trap(overflow)`).
...
- argv/env: `corpus/os/args_cwd.lu`, `corpus/os/env_roundtrip.lu` at
  `phase: run` (native), corpus gate green.
- files: `corpus/fs/roundtrip.lu`, `corpus/fs/error_row.lu` at
  `phase: run` (native).
...
spec/01 header now declares grammar/1. The new §10 "Grammar
versioning" adds `[gram.version]`, `[gram.version.1]` (additive-only
until v0.2), `[gram.version.anchor]` (the anchors are the contract,
citing `[conf.anchor.stable]`), and `[gram.version.enforce]`
(spec-extract + CI sync as the enforcement). `cargo xtask
spec-extract` regenerated `spec/anchors.json` (310 anchors, 4 new);
`spec/grammar.ebnf` is unchanged, because no EBNF was touched.
...
- lupin currently prints `lupin 0.1.6 (wolf-interp, pin e94b879)`, so
...
- wolf-lang: `ci success` at trunk `e94b879` (== origin/trunk). The
  release sha moves when this sprint and s71 land; re-verify then.
...
- Row 3 GREEN: diff-run at the pairing = zero unexplained
  divergences. DIV-2026-016 is unreproducible at the release sha,
  held open with evidence, and not carried as a blocker.
