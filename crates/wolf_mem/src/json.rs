//! The s40 json reference implementation (RFC 8259) — the checked
//! lane's single source of truth for the `json_*` builtin tier, and
//! the semantics a future native mirror pins itself to (the
//! `fmtspec` convention: one reference, shims mirror it, a parity
//! test pins the two).
//!
//! Scope is the *query-kernel* tier std.x.json wraps (D31 — the
//! nursery member delegates here for validation and path queries; the
//! DOM-into-region, reflection serde, and streaming reader are the
//! std facade's own, over D29/D10 machinery this module deliberately
//! does not model):
//!
//! - full RFC 8259 parse: objects, arrays, strings with every escape
//!   (`\uXXXX` including surrogate pairs), numbers, literals; one
//!   value per text, surrounding insignificant whitespace only;
//! - dotted-path queries ("users.0.name"; `""` is the root; a
//!   segment of digits indexes an array, anything else keys an
//!   object — keys containing `.` are unreachable at this tier,
//!   documented);
...
//! nothing), `Kind` (a node of the wrong kind for the operation).
//! Nesting deeper than [`MAX_DEPTH`] is a `Parse` error — RFC 8259
//! §9 allows implementations to set depth limits; ours is explicit.

/// Maximum container nesting the parser will descend (RFC 8259 §9).
pub const MAX_DEPTH: usize = 128;

/// Why a json operation failed.
