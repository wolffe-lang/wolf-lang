...
//! sound. A use the body PROVABLY makes outlive the call (it is
//! returned, or assigned away, or handed on `take`/`mut`) is `Opaque`
//! too. Until wolf-lang#387 it was its own verdict, `Escapes(Span)`,
//! reported as W1004 — the copy the analysis PROVED necessary, said
//! once instead of silently costing (s92; through s91 it was E1015, a
//! refusal). s165's wolf-lang#366 then made every such escape of a
//! `read` parameter a refusal in the CALLEE (E1002/E1014), so the
//! warning either fired beside an error for one root cause (VOICE
//! rule 5) or, on the one legal shape left — a local that dies with
//! the call — claimed something untrue. Both retired with #387.
//!
//! Misclassifying an escape as opaque costs nothing at all now that
//! no diagnostic rides on the distinction, and it never cost safety:
//! soundness rests entirely on `Lendable`'s whitelist.
//!
//! # The fix ladder
//!
...
//! passed anywhere. A callee that wants its own copy instead spells
//! `copy bs`, which is a READ position (wolf-lang#387): the call still
//! lends for free, and the copy happens once, inside the callee, where
//! the program spells it.
...
pub enum Lend {
    ...
    Lendable,
    /// Some use is outside the modelled surface, or provably outlives
    /// the call: materialize (this is the pre-s89 behaviour, and it is
    /// never wrong — only slower). A provable escape was its own
    /// verdict, `Escapes(Span)`, until wolf-lang#387 retired it with
    /// W1004; both answers always compiled to the same copy.
    Opaque,
}
...
impl<'a> Lender<'a> {
    ...
    fn param_at(&self, fsig: &FnSig, ix: usize, stack: &[(Span, usize)]) -> Lend {
        ...
        match (scan.escape, scan.opaque) {
            // wolf-lang#387: a proved escape and an unmodelled use are
            // one verdict now. The caller materializes for both, which
            // is what it always did — only the warning differed.
            (Some(_), _) | (None, true) => Lend::Opaque,
...
impl Scan<'_, '_> {
    ...
    fn node(&mut self, n: &GreenNode, escaping: bool) {
        ...
        match n.kind {
            ...
            // wolf-lang#387: `copy v` is a READ of the view. The callee
            // materializes a fresh `List[byte]` right here, and what
            // leaves afterwards is that copy — never the lend — so the
            // parameter stays lendable and the call still costs
            // nothing. Sound only since wolf-lang#384: before it,
            // native `copy` lowered as the operand itself, so the
            // "copy" handed back a list aliasing the string's own
            // immutable bytes.
            SyntaxKind::PrefixExpr => {
                let is_copy = n
                    .tokens()
                    .next()
                    .is_some_and(|t| t.kind == SyntaxKind::CopyKw);
                if is_copy
                    && wolf_ast::PrefixExpr::cast(n)
                        .and_then(|d| d.operand())
                        .is_some_and(|o| self.is_param(o))
                {
                    return;
                }
