...
fn corpus_parse_expectations() {
    ...
    // The ledger: 17 own-file syntax-tier counter-examples exist today
    ...
    // production now spells — and r05's STRING twin of it, the same
    // E0101 seven digits inside `"…"` — wolf-lang#198, where the
    // bound was asserted in prose over a `STR_PART` that derived no
    // escape at all — and s131's three D67 separator pins:
    ...
    // separator and a corpus file must fully format), and r06's E0101
    ...
    // wolf-lang#287); plus s151's three E0201 refusals of the bare `if`
    // (grammar/if_then_missing — neither `{` nor `then` after the
