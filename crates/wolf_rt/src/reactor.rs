...
///
/// This is deliberately NOT the reactor's poller above. That set is
...
/// rung is built on it). The call lives HERE, beside the poller, so
/// every per-OS io surface in the runtime stays in one module.
///
...
/// an answer.
#[cfg(unix)]
pub(crate) fn poll_readable(fds: &[RawFd], timeout_ms: i32) -> std::io::Result<Vec<bool>> {
