...
code!(E0413, "the format spec does not fit this value's type", r#"
...
or upstream moving off the excluded version. At v0 transport there is
no version list to fall back over, so the exclusion is a refusal, not
a silent downgrade; registry-side pick-the-next-minimum arrives with
the hosted transport (c15).
"#);
